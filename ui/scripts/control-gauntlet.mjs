...
const result = await build({
  stdin: { contents: `export { reviewSnapshot } from './src/lib/reviewSnapshot'; export { reorderWindowPrompts } from './src/lib/reorderWindowPrompts'; export { resolvedGenerationPlan } from './src/lib/generationPlan';`, resolveDir: process.cwd(), loader: 'ts' },
  bundle: true, write: false, format: 'esm', platform: 'node',
})
const { reviewSnapshot, reorderWindowPrompts, resolvedGenerationPlan } = await import(`data:text/javascript;base64,${Buffer.from(result.outputFiles[0].text).toString('base64')}`)
